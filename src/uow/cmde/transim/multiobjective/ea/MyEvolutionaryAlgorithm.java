 ...
 */
package uow.cmde.transim.multiobjective.ea;
...
/**
 * The {@link MyEvolutionaryAlgorithm} is an implementation of an Evolutionary
 ...
 * 
 */
public class MyEvolutionaryAlgorithm extends AbstractOptimizer {
	...
	/**
	 * Constructs an {@link MyEvolutionaryAlgorithm} with a {@link Population}, an
	 ...
	 */
	@Inject
	public MyEvolutionaryAlgorithm(Population population, Archive archive, IndividualFactory individualFactory,
			IndividualCompleter completer, Control control, Selector selector, Mating mating, Iteration iteration,
			@Constant(value = "alpha", namespace = MyEvolutionaryAlgorithm.class) int alpha,
			@Constant(value = "mu", namespace = MyEvolutionaryAlgorithm.class) int mu,
			@Constant(value = "lambda", namespace = MyEvolutionaryAlgorithm.class) int lambda) {
	...
	public void optimize() throws TerminationException, StopException {
		...
		while (iteration.value() < iteration.max()) {
			...
			for(Individual individual: population){
				...
				//{
					//System.out.println(iteration.value() + ":" + individual.getObjectives().toString());
			...
			if(!st.equals(""))
			{
				...
				//System.out.println("first generation:" + st);
				MOHandler.writeSolutionToFile("" + iteration.value(), st);
			}
