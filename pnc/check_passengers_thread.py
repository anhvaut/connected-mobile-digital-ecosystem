...
# version 3.0 : Vu Tran 2012 May, updating passenger counting system
# version 4.0 : Vu Tran 2013 March, upgrading passenger counting system
...
class GpsPoller(threading.Thread):
   ...
   def run(self):
       ...
       while 1:
			...
			globalLatitude = self.latitude
			globalLongitude = self.longitude
			globalSpeed = self.speed
...
def sendCommand(ser):
	...
	while True:
		#print 'send:'
		ser.write(PCNCommand.GET_COUNTERS)
		sleep(1)
...
def writePassengerDataToDB(longitude, latitude, on, quantity):
	...
	conn = mdb.connect('localhost', 'root', 'hollyzhu', 'busLocation');
	cursor = conn.cursor()
	cursor.execute(sqlInsert % (hostname, longitude,latitude,(datetime.datetime.now()),on,quantity ))
	conn.commit()
	cursor.close()
	conn.close()

#=================================
# sync passenger on/off data from local DB to Server DB
#=================================
def syncPassengerData():
	while True:
		try:
			...
			logger.debug("__Passenger Data syncing fail to send data to server")
