			...
			if(len(data)>0):
				...
				if (commandAnalyzer.getCommandName() == 'gcounters'):
					...
					#	print tmpSt
						...
						if(passengerOn !=0):
							busClient.service.insertpassengerMAC(hostname, stopLongitude, stopLatitude,str(datetime.datetime.now()),1,str(passengerOn))
							
						if(passengerOff !=0):
							busClient.service.insertpassengerMAC(hostname, stopLongitude, stopLatitude,str(datetime.datetime.now()),0,str(passengerOff))
